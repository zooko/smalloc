...
fn help_test_overflow_to_other_slab(sc: u8) {
    ...
    assert_eq!(sc1, sc);
    assert_eq!(slotnum1 as usize, i, "i: {i}, numslots: {numslots}, sc: {sc}, p1: {p1:?}, sc1: {sc1}, slabnum1: {slabnum1}, slotnum1: {slotnum1}, SLOTNUM_AND_DATA_MASK: {SLOTNUM_AND_DATA_MASK:b}");
...
nextest_unit_tests! {
    fn test_reqali_to_sc() {
        let test_cases = [
            (1, 1, max(2, NUM_UNUSED_SCS)),
            (2, 1, max(2, NUM_UNUSED_SCS)),
            ...
            (1, 2, max(2, NUM_UNUSED_SCS)),
            (2, 2, max(2, NUM_UNUSED_SCS)),
    ...
    fn slotnum_encode_and_decode_roundtrip() {
        for sc in [ 31, 30, 25, 12, 9, 5, 4 ] {
            ...
            let highestslotnum = (numslots - 1) as u32;
            assert_eq!(numslots - 1, highestslotnum as usize, "sc: {sc}");
            ...
            for slotnum1 in slotnums {
                ...
                for slotnum2 in slotnums {
                    ...
                    if slotnum1 < (numslots - 1) && (slotnum2 as usize) < numslots && slotnum1 != slotnum2 {
                        let ence = Smalloc::encode_next_entry_link(slotnum1 as u32, slotnum2 as u32, highestslotnum);
                        let dece = Smalloc::decode_next_entry_link(slotnum1 as u32, ence, highestslotnum);
                        assert_eq!(slotnum2 as u32, dece, "slotnum1: {slotnum1}, slotnum2: {slotnum2}, ence: {ence}, sc: {sc}, highestslotnum: {highestslotnum}");
                    }
                }
            }
        }
