...
pub fn main() {
    ...
    if compare {
        if thorough {
            ...
            if !is_macos() {
                compare_hs_bench!(one_ad, 100, THREADS_THAT_CAN_FIT_INTO_SLABS-1, iters_many, num_batches);

                // Windows ran out of resources (i.e. threads) running these:
                if !is_windows() {
                    compare_hs_bench!(one_ad, 200, THREADS_THAT_CAN_FIT_INTO_SLABS-1, iters_many, num_batches);
                    compare_hs_bench!(one_ad, 400, THREADS_THAT_CAN_FIT_INTO_SLABS-1, iters_many, num_batches);
                }
                compare_hs_bench!(a, 100, THREADS_THAT_CAN_FIT_INTO_SLABS-1, ITERS_FEW, num_batches);
                // Windows ran out of resources (i.e. threads) running these:
                if !is_windows() {
                    compare_hs_bench!(a, 200, THREADS_THAT_CAN_FIT_INTO_SLABS-1, ITERS_FEW, num_batches);
                    compare_hs_bench!(a, 400, THREADS_THAT_CAN_FIT_INTO_SLABS-1, ITERS_FEW, num_batches);
                }
            }
